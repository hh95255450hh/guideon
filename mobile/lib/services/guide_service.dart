...
/// Read-only access to the public guide/company catalogue.
class GuideService {
  /// Fetches guides, optionally filtered by a free-text [query] (destination,
  /// language, specialisation — the backend handles matching).
  static Future<List<Guide>> search({String? query, int limit = 24}) async {
  ...
  static Future<Guide?> byId(String id) async {
    ...
    if (data is Map && data['guide'] is Map) {
      return Guide.fromJson(
          (data['guide'] as Map).cast<String, dynamic>());
    }
