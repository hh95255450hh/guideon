# Guideon — معلومات المشروع الكاملة
**تاريخ الإنشاء:** 2026-05-17

---

## 🌐 الروابط

| الخدمة | الرابط |
|--------|--------|
| الموقع (Oman Data Park VPS) | https://guideon.om — نشر: `bash /opt/deploy.sh` على 185.64.25.111 |
| GitHub Repository | https://github.com/hh95255450hh/guideon |
| Supabase Dashboard | https://supabase.com/dashboard/project/uwgkszszsogivhphlfdy |
| Supabase REST API | https://uwgkszszsogivhphlfdy.supabase.co/rest/v1/ |
| Resend Dashboard | https://resend.com/domains |
| Hostinger DNS | https://hpanel.hostinger.com |

---

## 👤 حسابات الدخول

| النوع | الإيميل | الباسورد |
|-------|---------|----------|
| Admin | hh92hh@guideon.om | (غير معروفة — يلزم إعادة تعيين) |
| سائح (James) | tourist@example.com | Password123! |
| سائح (Anna) | anna@example.com | Password123! |
| سائح (Yuki) | yuki@example.com | Password123! |
| مرشد (Mohammed) | mohammed@Guideon.om | Password123! |
| مرشد (Fatima) | fatima@Guideon.om | Password123! |
| مرشد (Khalid) | khalid@Guideon.om | Password123! |
| مرشد (Aisha) | aisha@Guideon.om | Password123! |

---

## 🔑 API Keys & Secrets

| الخدمة | المفتاح |
|--------|---------|
| Supabase URL | https://uwgkszszsogivhphlfdy.supabase.co |
| Supabase Anon Key | sb_publishable_e7f2BmlvGIxIs3Ya1jyKrQ_hqW7X-jM |
| Resend API Key | re_43dnbDNB_C59XJtiSgQcWMvzxQFoac1qi |
| Session Secret | Guideon_Prod_Secret_2025 |

---

## 📧 إعدادات الإيميل (Resend)

**الحالة:** بانتظار التحقق من domain guideon.om

### سجلات DNS المطلوبة في Hostinger

| Type | Name | Value | Priority |
|------|------|-------|----------|
| TXT | `resend._domainkey` | `v=DKIM1; k=rsa; p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCbuPm7+j+lf2ablc75RnVoYF7Hj1m8XTfCU7TXP1LU05kdExbVCkUf6oOnHHFe2LZTzEnb83lrXDV3rkjKuOP9C0H9RG+dhHHmpZqjeA1qg1oQPu6l0052P+ZapDwV4MiIKomWCKJFloZxV2gFPD8flQIXblis/fF2JflnzebP7QIDAQAB` | - |
| TXT | `@` | `v=spf1 include:amazonses.com ~all` | - |
| TXT | `_dmarc` | `v=DMARC1; p=none;` | - |
| MX | `bounces` | `feedback-smtp.ap-northeast-1.amazonses.com` | 10 |

### إيميلات تُرسَل تلقائياً

| الحدث | المستقبل |
|-------|---------|
| تسجيل سائح جديد | السائح |
| تسجيل مرشد جديد | المرشد + الأدمن |
| إرسال طلب حجز | السائح + المرشد |
| تأكيد الحجز | السائح + المرشد |
| إلغاء الحجز | السائح + المرشد |
| إكمال الجولة | السائح (تذكير بتقييم) |
| إضافة مراجعة | المرشد |
| تحقق من حساب المرشد | المرشد |

---

## 🚀 Railway — متغيرات البيئة

| Key | Value |
|-----|-------|
| NODE_ENV | production |
| PORT | 3000 |
| SESSION_SECRET | Guideon_Prod_Secret_2025 |
| SUPABASE_URL | https://uwgkszszsogivhphlfdy.supabase.co |
| SUPABASE_ANON_KEY | sb_publishable_e7f2BmlvGIxIs3Ya1jyKrQ_hqW7X-jM |
| RESEND_API_KEY | re_43dnbDNB_C59XJtiSgQcWMvzxQFoac1qi |
| EMAIL_FROM | Guideon <noreply@guideon.om> |
| EMAIL_REPLY_TO | info@guideon.om |
| ADMIN_EMAIL | Hh95255450hh@hotmail.com |

> ✅ النطاق guideon.om مُحقَّق في Resend — الإرسال من `noreply@guideon.om` والردود تذهب لـ `info@guideon.om` عبر Cloudflare Routing.

---

## 🗄️ قاعدة البيانات (Supabase)

| الجدول | الوصف |
|--------|-------|
| users | جميع المستخدمين (أدمن، سياح، مرشدين) |
| bookings | الحجوزات |
| reviews | المراجعات |
| trip_requests | طلبات الرحلات |

---

## ✅ آخر التحديثات — 2026-07-05 (اكتمال إشعارات واتساب + قالب + تذكير + بريد)

- **قالب واتساب `guideon_alert`** (Utility, عربي, متغيّرين) معتمد ومربوط
  (`WHATSAPP_TEMPLATE_NAME=guideon_alert`, `LANG=ar`) — يخدم كل الإشعارات عبر
  `notify()` تلقائياً (تأكيد/إلغاء/تحديث/رسائل) واتساب + بريد + داخل المنصّة.
- **تذكير قبل الجولة بـ12 ساعة:** `src/services/reminderScheduler.js` (يبدأ مع
  التطبيق، يعمل كل ساعة). اختبار يدوي: `/api/admin/reminder-test?bookingId=`.
- **بريد admin@guideon.om:** كان لا يصل بسبب حجر M365 للانتحال الذاتي (Resend
  يرسل باسم النطاق إلى صندوق على نفس النطاق) — حُلّ بإطلاق المُرسِل من الحجر.
  اختبار: `/api/admin/email-test?to=...` (verbose).

---

## ✅ آخر التحديثات — 2026-07-05 (واتساب حيّ + نشر تلقائي + إصلاحات جوال)

- **WhatsApp Cloud API — حيّ الآن:** رُبط حساب WABA الجديد (المعرّف `1342917324642763`)
  بالرقم العُماني الحقيقي **+968 7419 7197**. المتغيّرات في `.env` على الخادم:
  `WHATSAPP_ENABLED=true`, `WHATSAPP_PHONE_NUMBER_ID=1229241020270519`,
  `WHATSAPP_ACCESS_TOKEN` (**رمز دائم** من System User `guideon-system`, لا ينتهي),
  `WHATSAPP_TEMPLATE_NAME=hello_world`, `WHATSAPP_TEMPLATE_LANG=en`.
  اختبار الإرسال `GET /api/admin/whatsapp-test?to=...&mode=text` رجع `ok:true`.
  لا يوجد مستقبِل webhook للرسائل الواردة (إرسال فقط حالياً).
- **نشر تلقائي (webhook داخل ODP):** كل push إلى main يُنشر ذاتياً عبر خدمة
  `guideon-deploy-hook` (systemd) — لا حاجة لـSSH يدوي بعد الآن.
  **إصلاح (2026-07-15):** كان الـ webhook يُطلق لكن النشر يفشل — الغلاف القديم
  كان يسبق `deploy.sh` بـ`git commit --no-edit` (تراكم 84 commit دمج على الخادم
  وخطر تعارض في `deploy/*`) ومهلة 20 دقيقة قد يتجاوزها بناء بارد (تنزيل chromium
  + npm ci). الآن الغلاف يشغّل `bash /opt/deploy.sh` فقط (deploy.sh يتكفّل بالسحب
  + بناء 3 محاولات + up + إعادة nginx + فحص صحة + تنبيه بريدي تحت flock) بمهلة
  40 دقيقة. شجرة git على الخادم نُظّفت لتصبح السحبات fast-forward نظيفة.
- **جوال:** إصلاح القائمة الجانبية (☰) في الوضع العربي (خطأ أولوية CSS)،
  زر لغة ثابت (ع/EN) بجانب الجرس، وإصلاح أيقونة الخروج العملاقة.
- **تصميم:** فوتر الرئيسية أصبح مينيمال (أزرار رئيسية)، وبطاقات KPI/شارات/Avatar
  في جداول الأدمن ولوحة الشركة.

---

## ✅ آخر التحديثات — 2026-07-05 (تعديل الأدمن للملفات + بداية نظام التصميم)

- **تعديل كامل من الأدمن لملفات المرشدين والشركات:** نقطة `PATCH /api/admin/users/:id`
  (صلاحية `edit_users`) وُسّعت لتقبل **كل حقول الملف الشخصي** (نصوص، أرقام، منطقية،
  مصفوفات نصية، وحقول JSON بنيوية) مع حفظ مرن يُسقط أي عمود غير موجود ويُعيد المحاولة،
  وتسجيل كامل في سجل التدقيق. مستبعَد: كلمة المرور والتوكنات ونوع الحساب والتقييم.
- **واجهة:** زر ✏️ بجانب كل مرشد وشركة في لوحة الأدمن يفتح نافذة تعديل تبني الحقول
  تلقائياً حسب نوع الحساب وتحفظ عبر الـAPI ثم تُحدّث القائمة.
- **نظام التصميم:** ملف `public/css/design-system.css` (توكنات ألوان/خطوط/مسافات +
  أصناف `gd-*` قابلة لإعادة الاستخدام: بطاقات KPI، شارات حالة، أزرار أيقونات موحّدة،
  Avatar، جداول مصقولة). يُحمَّل على كل الصفحات عبر `common-widgets.js` (رُفع إلى `?v=24`).
  أُعيد تصميم أزرار إجراءات جداول المرشدين/الشركات لتصبح موحّدة (`gd-iconbtn`).
- **توثيق:** مولّدا الوثائق (`scripts/generate-docs.js` و`_gen_credentials_doc.js`)
  استُبدلت فيهما تعليمات Railway القديمة ببنية Oman Data Park VPS (نشر عبر SSH و
  `bash /opt/deploy.sh`، سجل A → 185.64.25.111، nginx + Docker Compose).

---

## ✅ آخر التحديثات — 2026-07-04 (المجلس المُوسّع — جودة وأمان وتنظيف شامل)

عُقد المجلس بمسؤولين إضافيين (QA وظيفي يتتبّع رحلة المستخدم + مسؤول تنظيف الكود)
ونُفّذت الإصلاحات على 6 دفعات، كلها منشورة ومُتحقَّق منها على الخادم:

- **الدفعة 1 (أمان):** إصلاح XSS مُخزَّن في لوحة الأدمن (`jattr` + `gdEsc`)، وقائمة
  المشاركة العامّة، وصفحة تخطيط الرحلة؛ إخفاء بريد/هاتف السائح من نتائج المطابقة؛
  حذف ملفّات `src/data/*.json` التي كانت تحوي بيانات شخصية (إيميلات/كلمات مرور).
- **الدفعة 2 (صحّة المنطق):** حماية فلاتر البحث من `NaN` (سعر/تقييم/أيام)، ومطابقة
  الصعوبة بلا حسّاسية لحالة الأحرف، ومزامنة عدّاد تقييمات الباقات.
- **الدفعة 3 (i18n + إصدارات):** إضافة 13 مفتاح ترجمة ناقص (عربي/إنجليزي)،
  ورفع `PACK_VERSION` وإصدارات الأصول لمنع التخزين المؤقّت القديم.
- **الدفعة 4 (إتاحة/Accessibility):** تقويم قابل للتشغيل بلوحة المفاتيح
  (Enter/Space + `aria-label`)، وإطار تركيز مرئي `:focus-visible` عامّ، ورابط
  "تخطّي إلى المحتوى"، وتغميق اللون الذهبي ليحقّق تباين WCAG AA على الأزرار.
- **الدفعة 5 (سلامة البيانات):** قائمة أماكن عُمان المعتمدة
  (`src/utils/omanPlaces.js`) تُسقِط أيّ وجهة خارج السلطنة (مثل "موسكو") عند
  التسجيل وتحديث الملفّ؛ وبوّابة اكتمال الملفّ قبل توثيق المرشد (سيرة/وجهات/لغات/
  سعر) مع إمكانية تجاوز يدوي `force`.
- **الدفعة 6 (تنظيف):** حذف `railway.toml` و`web_app_screen.dart` الميّتين.

**الاختبارات:** 70 ناجحة.

### ⚠️ يتطلّب إجراءً يدويّاً منك (سجلّات في قاعدة البيانات لا يمكنني تعديلها)
- **حساب مرشد بوجهة خارج عُمان** (`guide-65d4d513`, موسكو/روسيا): عدّل وجهاته من
  لوحة الأدمن أو احذف الحساب. (الحماية البرمجية تمنع تكرار المشكلة مستقبلاً فقط.)
- **مرشد مكرّر** باسم "Mohand AlBusaidi": ادمج/احذف النسخة الزائدة.
- **باقتان بلا صورة غلاف:** أضِف صورة غلاف أو أخفِهما.

---

## ✅ آخر التحديثات — 2026-06-30 (إصلاحات المجلس الاستشاري — 6 إصلاحات أمنية وميزات جديدة)

### 🔒 إصلاحات أمنية
- **Rate limit على guest-checkout:** `guestCheckoutLimiter` (5 محاولات/10 دقائق) — يحمي من spam وإغراق البريد.
- **أرقام عربية تتجاوز الفلتر:** `PHONE_RE` صار `\p{N}` (Unicode) بدل `\d` (ASCII فقط) — يمسك ٩٦٧٩٥٢... وما شابهها.
- **تصليب ownership في updateStatus:** أضيف فحص صريح لأنواع المستخدمين غير المعروفة (`team`، `staff`) → 403.
- **Idempotency للـ webhooks (migration 052):** `UNIQUE INDEX` على `paymentRef` + معالجة خطأ 23505 في `finalizePaidBooking` — لا تُعالَج مدفوعات مرتين.

### 🎁 نظام الإحالة (Referral) — migration 053
- كل مستخدم جديد يحصل على `referralCode` فريد (6 أحرف).
- رابط الإحالة: `guideon.om/register.html?ref=XXXX` — يُمرَّر للـ API تلقائياً.
- المُحال يحصل على **خصم 5%** (بحدٍّ أقصى 5 OMR) في أول حجز تلقائياً.
- بطاقة الإحالة تظهر في لوحة السائح مع روابط واتساب + بريد.

### 📊 Funnel Analytics — migration 054
- جدول `analytics_events` لتتبع: `book_click` → `checkout_start` → `guest_checkout_start` → `guest_checkout_complete`.
- دالة `window.gdTrack()` في `common-widgets.js` — fire-and-forget، لا تعطّل الصفحة.
- نقطة `POST /api/analytics/event` (عامّة، قائمة بيضاء للأحداث المسموح بها).

### ⚠️ خطوات يدوية مطلوبة في Supabase ✅ تمّت
- **migration 052** (payment idempotency) ✓
- **migration 053** (referral system) ✓
- **migration 054** (analytics events) ✓

---

## ✅ آخر التحديثات — 2026-06-30 (الحجز كزائر — Guest Checkout)

- **حجز سريع دون حساب (مثل jawla.om):** عند الضغط على "ادفع واحجز" في أي رحلة، إذا لم يكن المستخدم مسجّلاً يظهر نموذج صغير (الاسم + البريد/الهاتف) بدل تحويله لصفحة التسجيل → يدفع مباشرة ويصله رابط الحجز على بريده.
- **النقطة الطرفية:** `POST /api/payments/guest-checkout` (عامّة، بلا تسجيل دخول). تتحقّق من الاسم + (بريد أو هاتف)، تُنشئ الحجز بـ `touristId = null`، ثم تبدأ الدفع (Paymob/Thawani) وتُعيد `url`.
- **قاعدة البيانات (هجرة 051):** أعمدة `guestName/guestEmail/guestPhone` على جدول `bookings` + فهرس على البريد. طُبّقت في الإنتاج ✓.
- **الإشعارات:** المرشد يُخطَر بالحجز بعد الدفع حتى للحجوزات بلا حساب؛ والزائر يصله بريد تأكيد فيه رابط حجزه (`checkout-success.html`).
- **التطبيقات:** التطبيق غلاف WebView لـ guideon.om فالميزة تعمل فيه تلقائياً دون إصدار جديد.

---

## ✅ آخر التحديثات — 2026-06-25 (فواتير المزوّدين + طلب الدفع + AI للمرشد + تدقيق تجاوب شامل)

- **إصلاح حرج — الفواتير لا تفتح:** كان محرّك الـPDF (Puppeteer) يُرجِع `Uint8Array` بدل `Buffer` فيُفسد الملفّ ("can't open this file"). أُصلح في `pdfDocs.htmlToPdf` ويُصلح كل الفواتير/الإيصالات/الكشوف.
- **قسم فواتير مخصّص (مرشد + شركة):** تفاصيل كل فاتورة + حالة (✅ مدفوعة / 💰 مستحقّة / ⏳ قيد المراجعة / 🚌 قيد التنفيذ) + تحميل PDF.
- **زرّ "💸 طلب الدفع":** المزوّد يطلب صرف مستحقّ منسيّ → يُسجَّل `payoutRequestedAt` (هجرة 049) ويصل تنبيه بريد للمنصّة. نقاط المحاسبة صارت `requireProvider` (الشركات أيضاً).
- **AI للمرشد:** زرّ "✨ حسّن وصف الرحلة بالذكاء" في محرّر رحلة المرشد (كان للشركات فقط) — يعمل على الهاتف.
- **تدقيق تجاوب شامل:** فُحصت كل الصفحات (38 عامّة + لوحات التحكّم) آليّاً عند 390px و768px — **لا فيض أفقي في أيّ صفحة**؛ وأُصلح التفاف عمود الوجهة في سجلّ المرشد.
- **بنية النشر (ODP VPS):** اكتُشف أنّ بناءات Docker متزامنة متراكمة كانت تتجمّد على قفل buildkit وتمنع تحديث الحاوية — تُشغَّل الآن بناءً واحداً نظيفاً.

---

## ✅ آخر التحديثات — 2026-06-12 (لوحة الإيرادات + الإدارة المالية + إصلاح حرج للبحث)

### 🚨 إصلاح حرج: عودة البحث وقوائم الأدمن
تحسين أداء سابق (M1) دفع الترتيب + `count:'exact'` + التقطيع إلى Postgres عبر `SupabaseDB.findPage`، لكنه **أرجع 0 نتيجة على بيانات الإنتاج** — أفرغ بحث المرشدين (`/api/guides`) وكل قوائم الأدمن بصمت.
- **الإصلاح:** الرجوع إلى `findAllWhere`/`readAll` + ترتيب/فلترة/تقطيع في JS (المسار المُجرَّب).
- **الملفّات:** `searchGuides` (commit 6426c9e) + `userPage` للأدمن (commit 47fd720).
- **درس:** أيّ نقل منطق إلى قاعدة البيانات يجب اختباره على بيانات إنتاج حقيقيّة — الاختبارات المحليّة بلا DB لا تكشفه.

### 💰 لوحة تحكّم الإيرادات (`/admin-revenue.html`)
لوحة تحليلات ماليّة احترافيّة (وضع ليلي افتراضي، ApexCharts، متجاوبة):
- **نظرة عامّة:** إجمالي/يومي/أسبوعي/شهري/سنوي + نموّ% + عدد العمليّات + متوسّط قيمة الطلب + أرباح المنصّة + مستحقّات المزوّدين.
- **رسوم:** خطّ زمني (يومي/شهري) + توزيع حسب المصدر (أفراد/شركات) + طرق الدفع.
- **جداول:** أعلى المزوّدين دخلاً + الإيراد حسب الوجهة + سجلّ عمليّات (بحث/فلترة/فرز).
- **تنبيهات ذكيّة:** كشف انخفاض/ارتفاع الإيراد + ضغط فشل الدفع.
- **تصدير:** CSV (مع BOM للعربي) + طباعة PDF.
- **Backend:** `GET /api/admin/revenue` (صلاحيّة `view_analytics`، cache 30s).

### 🏦 الإدارة المالية — المصروفات والرواتب
- جدول `finance_expenses` (migration **037** — يجب تشغيله في Supabase).
- 10 تصنيفات: راتب · خصم · تسويق · تشغيلي · إيجار · برمجيّات · استرداد · دفعة مزوّد · ضرائب · أخرى.
- خانة "شهري متكرّر" للرواتب/الإيجار.
- **صافي الربح** = أرباح المنصّة (العمولة) − المصروفات + هامش%.
- **Endpoints:** `GET/POST/PATCH/DELETE /api/admin/expenses` (صلاحيّة `manage_finance` للكتابة) — كلّها مُسجَّلة في Audit Log.
- صلاحيّة جديدة `manage_finance` (الأدمن + دور Finance).

### ⚡ تحسينات أداء (M3 + M4)
- **M3:** توحيد كود الرسائل المكرّر (~576 سطر) في `public/js/gd-messages.js` المشترك بين الـ3 dashboards.
- **M4a:** تقسيم `i18n.js` (167KB) → core 6KB + 10 ملفّات JSON تُحمَّل حسب اللغة (`public/i18n/*.json`). توفير 57-60% لزوّار الإنجليزيّة/الصينيّة/الروسيّة.
- **M4b:** كل صفحة تحمّل اتّجاه Bootstrap واحد فقط (LTR أو RTL) بدل الاثنين — توفير ~30KB/صفحة.
- **Cache:** الأصول الثابتة (CSS/JS/صور) تُخزَّن سنة كاملة (`immutable`)، HTML يبقى `no-store`.
- الاختبارات: 12 → **46 اختبار**.

### 🇴🇲 تحسينات أخرى
- علم عُمان الرسمي (SVG: خنجر + سيفان) في الـfooter.
- زرّ + بطاقة + تبويب للوصول للوحة الإيرادات في `admin.html`.

### ⚠️ خطوة يدويّة مطلوبة في Supabase
تشغيل migration **037** (`database/migrations/037_finance_expenses.sql`) لتفعيل إضافة المصروفات.

---

## ✅ آخر التحديثات — 2026-05-30 (إصلاح حرج في الحجز والتقييم)

مراجعة شاملة للمنصة وإصلاح خطأين حرجين في تدفق الحجز والتقييم — أعمدة كان التطبيق يكتبها لكنها مفقودة من قاعدة البيانات.

| الخطأ | الأثر قبل الإصلاح | الإصلاح |
|---|---|---|
| جدول `bookings` ينقصه `startedAt`, `completedAt`, `variantName`, `addons` | المرشد لا يقدر يبدأ/ينهي الرحلة · حجوزات الباقات مع إضافات تفشل | دوال مرنة `insertBookingSafe`/`updateBookingSafe` + migration 021 |
| جدول `reviews` ينقصه `packageId`, `touristPhoto` | السائح لا يقدر يرسل أي تقييم (خطأ 500) | دالة مرنة `insertReviewSafe` + migration 022 |

### آلية الحماية
الدوال المرنة تتجاهل الأعمدة الاختيارية تلقائياً عند خطأ schema-cache فيكتمل الحجز/التقييم فوراً حتى قبل تشغيل الترحيل. تبقى كطبقة حماية دائمة ضد أي اختلاف مستقبلي في المخطط.

### ترحيلات جديدة (تم تشغيلها في Supabase ✓)
- `database/migrations/021_booking_lifecycle_columns.sql`
- `database/migrations/022_review_columns.sql`

### مسارات الكتابة الأخرى (فُحصت — سليمة)
التسجيل، إنشاء/تعديل الباقات، تعديل ملف المرشد — جميعها تحتوي على معالجة أخطاء كافية أو أعمدتها موجودة.

---

## ✅ آخر التحديثات — 2026-05-30 (إحصائيات حيّة حقيقية)

استبدال أرقام الصفحة الرئيسية الثابتة (المكتوبة يدوياً) بأرقام حقيقية ديناميكية من قاعدة البيانات.

| البند | التفاصيل |
|---|---|
| **نقطة API جديدة** | `GET /api/stats/public` — عامة بدون تسجيل دخول · كاش 5 دقائق في الذاكرة |
| **ما تحسبه** | مرشدون معتمدون (موثّقون وغير موقوفين) · وجهات مغطّاة (مختلفة عبر المرشدين) · متوسط التقييم الفعلي · سياح أتمّوا حجزاً |
| **الصفحة الرئيسية** | `index.html` يجلب الأرقام حيّاً عبر `loadStats()` مع إبقاء القيم القديمة كاحتياطي عند فشل الاتصال |

### ملفات جديدة
- **Controller:** `statsController.js`
- **Route:** `stats.js` (مربوط في `app.js` على `/api/stats`)

### تغييرات أخرى
- تغيير إيميل الأدمن من `admin@guideon.guide` إلى `hh92hh@guideon.om` (عبر `set-admin-email.js`)
- تحديث `ANDROID_APP_GUIDE.md`: اسم حزمة التطبيق الفعلي `guide.guideon.twa` (مطابق لـ assetlinks.json المنشور)

---

## ✅ آخر التحديثات — 2026-05-22 (ميزات مستوحاة من GetYourGuide/Viator/Airbnb)

8 ميزات جديدة تسد الفجوات مع المنافسين العالميين (GetYourGuide, Viator, Airbnb Experiences, Withlocals, ToursByLocals).

| الميزة | التفاصيل |
|---|---|
| **F1: SEO Suite** | sitemap.xml ديناميكي مع cache · robots.txt · OG/Twitter meta tags · JSON-LD structured data (Organization + Person + TouristTrip) عبر `/js/seo.js` |
| **F2: Tour Packages** | جدول tour_packages كامل · controller + 7 endpoints (list/get/create/update/delete/mine/feature) · صفحة `tour-package.html` تفاعلية مع itinerary وسعر متعدد المستويات (adults/children) |
| **F3: PDF Voucher** | pdfkit مع تصميم احترافي · `GET /api/bookings/:id/voucher` · يتضمن بيانات الحجز + معلومات الاتصال + شروط الإلغاء |
| **F4: Reviews with Photos** | حتى 3 صور لكل مراجعة (Supabase Storage) · helpfulCount field · `POST /api/reviews/upload-photo` |
| **F5: Q&A on guide profile** | جدول guide_questions · ask/answer/byGuide/forMe endpoints · إيميل تنبيه للمرشد · عرض عام للأسئلة المُجاب عنها |
| **F6: Wishlist Sharing** | جدول shared_wishlists · `POST /api/wishlist/share` ينتج رابط مشاركة · صفحة `shared-wishlist.html` |
| **F7: Newsletter** | جدول newsletter_subscribers · subscribe/unsubscribe endpoints · welcome email · popup widget يظهر بعد 8s مع dismiss للأبد عبر localStorage |
| **F8: Multi-tier Pricing** | price_adult + price_child + max_group_size في tour packages · حساب فوري للمجموع · cancellation_policy (flexible/moderate/strict) |

### ملفات جديدة
- **Routes:** `seo.js`, `packages.js`, `qa.js`, `extras.js`
- **Controllers:** `packageController.js`, `qaController.js`, `wishlistController.js`, `newsletterController.js`
- **Services:** `voucherService.js`
- **Frontend:** `tour-package.html`, `shared-wishlist.html`, `js/seo.js`, `js/newsletter.js`, `robots.txt`
- **Migrations:** `004_tour_packages.sql`, `005_reviews_with_photos.sql`

### حزم جديدة
- `pdfkit` لتوليد PDF vouchers

### Migrations يدوية مطلوبة في Supabase
1. `004_tour_packages.sql` — جدول tour_packages + package_availability + إضافة packageId/adultCount/childCount إلى bookings
2. `005_reviews_with_photos.sql` — photos array في reviews + جدول guide_questions + newsletter_subscribers + shared_wishlists

---

## ✅ آخر التحديثات — 2026-05-22 (إنتاج: المراحل 1-4 الكاملة)

ترقية المشروع من "يعمل" إلى **Production-Ready حقيقي**. 4 مراحل، 19 مهمة، حزم جديدة، اختبارات، CI، توثيق كامل.

### المرحلة 1 — الأساسيات الحرجة

| التحديث | التفاصيل |
|---|---|
| **حذف الكود الميت** | حذف 14 ملف + scripts قديمة + Tempcookie + dashboard.html + public/app.js + schema.sql القديم |
| **CORS whitelist** | تضييق origins عبر متغير CORS_ORIGINS بدل origin:true |
| **Rate Limiting** | login (5/15min), register (3/h), password reset (3/h), api (100/min), chat (20/min) |
| **SupabaseDB efficient queries** | findByField, findAllByField, count — SQL مباشر بدل readAll() + filter |
| **Postgres Session Store** | connect-pg-simple — الجلسات تستمر عبر redeploys |
| **Supabase Storage** | كل الصور تُرفع إلى Supabase Storage بدل filesystem |

### المرحلة 2 — تجربة المستخدم الأساسية

| التحديث | التفاصيل |
|---|---|
| **Password Reset Flow** | forgot-password.html + reset-password.html + إيميل احترافي + token expires بساعة |
| **Email Verification** | إيميل تأكيد عند التسجيل + verify-email/:token + resend-verification |
| **Booking Race Condition** | unique index على (guideId, tourDate) WHERE status IN active → معالجة 409 |
| **CSRF Protection** | Origin/Referer check + SameSite=strict في الإنتاج |

### المرحلة 3 — الجاهزية التشغيلية

| التحديث | التفاصيل |
|---|---|
| **Structured Logging** | pino مع redaction للأسرار + httpLogger بمدة الطلب |
| **Health endpoints** | /health/live, /health/ready (DB ping), /health/metrics (memory) |
| **Sentry integration** | اختياري عبر SENTRY_DSN — يتفعّل تلقائياً |
| **Tests** | 12 اختبار يعمل عبر node:test (npm test) |
| **Database indexes** | migration 003 — indexes لـ users/bookings/reviews/messages/trip_requests |
| **API Documentation** | README.md جديد كامل بالـ endpoints والـ env vars |
| **Validator standardization** | express-validator على endpoints المصادقة الحساسة |

### المرحلة 4 — تحسينات الجودة

| التحديث | التفاصيل |
|---|---|
| **req.user middleware** | loadUser middleware يلصق المستخدم على req.user تلقائياً |
| **CI/CD (GitHub Actions)** | .github/workflows/ci.yml — syntax + tests على Node 18 و 20 |
| **Global error handler** | يلتقط الأخطاء، يسجّلها في pino+Sentry، يرد JSON آمن |
| **uncaught handlers** | unhandledRejection + uncaughtException — لا يقع السيرفر صامتاً |

### الحزم الجديدة المُضافة
- @sentry/node, pino, pino-pretty
- (حُذف: connect-flash, docx, jsonwebtoken — كانوا غير مستخدمين)

### Migrations المُضافة (يجب تطبيقها في Supabase)
1. `database/migrations/002_booking_unique_constraint.sql`
2. `database/migrations/003_performance_indexes.sql`

### متطلبات .env الجديدة (اختيارية لكن مُوصى بها)
- `SENTRY_DSN` — مجاني من sentry.io (5K errors/شهر)
- `CORS_ORIGINS` — قائمة origins مفصولة بفواصل
- `SUPABASE_STORAGE_BUCKET` — اسم الـ bucket (افتراضي: media)
- `LOG_LEVEL` — info/debug/warn/error

### ⚠️ خطوات يدوية بعد النشر
1. **في Supabase Dashboard:** أنشئ Storage bucket باسم `media` واجعله Public
2. **في Supabase SQL Editor:** شغّل migration 002 و 003
3. **في Railway env vars:** أضف `CORS_ORIGINS` و `SENTRY_DSN` (اختياري)
4. **استبدل Stripe keys الـ placeholder** بمفاتيح حقيقية لتفعيل الدفع

---

## ✅ آخر التحديثات — 2026-05-22 (إصلاح أخطاء حرجة)

| التغيير | التفاصيل |
|---------|---------|
| **حذف الكود الميت** | حذف 3 controllers + 3 routes + 2 middleware + uploadService + notificationService + FileDB + config/database.js (كانت تستخدم نظام JWT + pg مختلف وغير مربوطة في app.js) |
| **إصلاح منطق الدفع** | paymentController.js: استخدام حقل isPaid بدل status لتحديد "تم الدفع" — الآن المرشد يمكنه تأكيد الحجز قبل الدفع دون كسر مسار الدفع |
| **إعادة التاريخ عند الإلغاء** | bookingController.js: عند إلغاء الحجز، التاريخ يُعاد إلى توفر المرشد (كما يَعِد إيميل الإلغاء) |
| **SupabaseDB.findById** | استخدام this.pk بدل القيم المُرمزة (id/reviewId) — يدعم الآن أي مفتاح أساسي مخصص |
| **إزالة المفاتيح المكشوفة** | config/supabase.js: حذف Supabase URL والمفتاح المُرمَزَين كـ fallback — الآن يرمي خطأ إذا غاب env |
| **try-catch في authController** | إضافة try-catch للدوال: updateProfile, changePassword, uploadPhoto, saveFcmToken (كانت تسبب unhandled rejections) |
| **حذف الجلب المضاعف** | bookingController.updateStatus: إزالة استعلام مرشد مكرر داخل بلوك confirmed |
| **حذف webhook مكرر** | routes/payments.js: حذف تسجيل /webhook المكرر — الأصلي مُسجّل في app.js مع express.raw() |
| **إصلاح seed script** | package.json: تصحيح "scripts/seed.js" → "scripts/seed_supabase.js" |

---

## ✅ آخر التحديثات — 2026-05-21 (تحديث 3)

| التغيير | التفاصيل |
|---------|---------|
| i18n كامل — جميع الصفحات | i18n.js v16: ~50 مفتاح جديد + ترجمة عربية لجميع العناصر الجديدة |
| guide-profile.html | كل النصوص الثابتة والديناميكية تدعم العربية/الإنجليزية عبر I18N.t() و data-i18n |
| company-dashboard.html | لوحة التحكم كاملة بالعربية مع window.onLangChange |
| admin.html | جدول المرشدين والشركات والسياح والحجوزات يتحول مع اللغة |
| admin.html + adminController.js | دعم الشركات + المرشدين غير المرخصين في لوحة الأدمن |
| company-dashboard.html | لوحة تحكم الشركات: ملف الشركة، الحزم السياحية، إدارة كاملة |
| guide-profile.html | صفحة ملف المرشد الكاملة للسياح: تقويم، حجز، مراجعات، ودية/مشاركة |
| emailService.js | إيميلات الترحيب والموافقة للشركات |
| authController.js | دعم تسجيل الشركات + isMinistryLicensed للمرشدين |
| **رفع الصور والفيديو** | src/routes/upload.js: POST /photo /gallery DELETE /gallery POST /video |
| guide-dashboard.html | انقر الأفاتار لتغيير الصورة + معرض صور (8 صور) + رابط YouTube |
| company-dashboard.html | انقر الشعار لتغيير الصورة + معرض صور + رابط YouTube في وضع التعديل |
| guide-profile.html | عرض معرض الصور + فيديو YouTube مضمّن في الصفحة العامة |
| إلغاء الموقع الإلكتروني للشركات | حذف companyWebsite من register.html + company-dashboard.html + authController.js |
| i18n v17 | مفاتيح gallery_* و video_* و photo_* بالعربية والإنجليزية |
| **الدردشة المباشرة بين السائح والمرشد** | جدول messages في Supabase + API كامل (send/thread/conversations/unread-count) |
| tourist-dashboard.html | قسم رسائل مع قائمة المحادثات + خيط رسائل + تحديث تلقائي كل 9 ثوانٍ |
| guide-dashboard.html | نفس قسم الرسائل للمرشد للرد على الرسائل |
| guide-profile.html | بطاقة "مراسلة المرشد" للسياح مع خيط رسائل مضمّن في الصفحة |
| i18n v18 | مفاتيح msg_* بالعربية والإنجليزية (18 مفتاح جديد) |

---

## 📁 هيكل المشروع

```
GUIDEON/
├── public/              ← الواجهة الأمامية (HTML, CSS, JS)
├── src/
│   ├── app.js           ← نقطة البداية
│   ├── config/
│   │   └── supabase.js  ← إعداد Supabase
│   ├── controllers/     ← منطق API
│   ├── models/
│   │   └── SupabaseDB.js ← نموذج قاعدة البيانات
│   ├── routes/          ← مسارات API
│   └── services/
│       └── emailService.js ← خدمة الإيميلات
├── scripts/
│   └── seed_supabase.js ← نقل البيانات
├── .env                 ← متغيرات البيئة (محلي)
└── ecosystem.config.js  ← إعداد PM2
```

---

## 🛠️ تشغيل المشروع محلياً

```bash
cd C:\Users\hh952\OneDrive\Desktop\oman\GUIDEON
node src/app.js
# الموقع: http://localhost:3000
```

## 📤 نشر تحديث جديد

```bash
cd C:\Users\hh952\OneDrive\Desktop\oman\GUIDEON
git add -A
git commit -m "وصف التغيير"
git push origin main
# Railway ينشر تلقائياً بعد الـ push
```

---

## 📱 المشروع

- **الاسم:** Guideon
- **الوصف:** منصة حجز مرشدين سياحيين معتمدين في سلطنة عُمان
- **GitHub:** hh95255450hh
- **الإيميل:** Hh95255450hh@hotmail.com
- **Domain:** guideon.om

---

## 📱 تطبيق الجوال الرسمي (Flutter — Android + iOS)

- **المصدر:** `mobile/` (Flutter، التطبيق يتّصل بـ`https://guideon.om`).
- **مُعرّف التطبيق:** `om.guideon.guideon` (موحّد للمنصّتين).
- **البناء السحابي:** Codemagic (`codemagic.yaml`) — 3 مسارات:
  - `android-test` → APK تجريبي فوري (بلا إعداد).
  - `android-play` → AAB لمتجر Google Play (يحتاج مجموعة `keystore_credentials`).
  - `ios-release` → IPA لمتجر Apple (يحتاج تكامل App Store Connect — بلا Mac).
- **الأيقونة + شاشة الإطلاق:** تُولَّد في البناء من شعار Guideon
  (`flutter_launcher_icons` + `flutter_native_splash`).
- **الإشعارات:** Firebase Cloud Messaging — الكود جاهز (`PushService`)، يُفعَّل
  تلقائيّاً بعد إضافة `google-services.json` / `GoogleService-Info.plist`
  (انظر `mobile/FIREBASE_SETUP.md`). الخادم يخزّن الرمز عبر `POST /auth/fcm-token`.
- **بيانات المتجر:** `mobile/STORE_LISTING.md` (وصف عربي/إنجليزي + روابط الخصوصية).
- **نوع حساب المطوّر:** مؤسّسة باسم "الرؤية للفكر الرقمي" (D-U-N-S 850403864).

---

## 🔧 مراجعة آليّة الحجز (043) — 2026-06-21

تدقيق عميق لمنطق الحجز/التوقيت كشف وأصلح:
- **القيد الفريد كان `(guideId, tourDate)`** → حجز واحد فقط لكل مزوّد/تاريخ، ما
  عطّل **تعدّد حجوزات الشركات** و**الرحلات الجماعية** و**عدّة فترات/فتحات في
  اليوم**. استُبدل بفهارس backstop للمرشد الفردي فقط (عمود جديد
  `bookings.providerType`)؛ الشركات والباقات تُحكَم بمنطق السعة في التطبيق.
- **رفض التواريخ الماضية وغير الصالحة** عند إنشاء الحجز.
- **منع بيع الباقة فوق `max_group_size`** (جمع المقاعد عبر الحجوزات).
- **توقيت عُمان (UTC+4)** لحسابات "اليوم" و48 ساعة وبدء الرحلة، وآمن ضد NaN.
- migration **043** مُطبّقة على الإنتاج؛ 59 اختباراً ناجحاً.

---

## 🛡️ تدقيق أمني شامل — 2026-06-22

فحص هندسي عميق للمنصّة كاملة. **أُصلح ونُشر:**
- **تجاوز دفع (Thawani):** ربط الجلسة بالحجز (`client_reference_id` + مطابقة المبلغ) في verify/webhook.
- **منع حجز المرشد لنفسه** (تضخيم حجوزات/تقييم وهمي).
- **سباق السعة المتزامنة للشركات:** مصالحة حتميّة بعد الإدراج (`bookingsWithinCapacity`).
- **توحيد نظامَي التوفّر:** مرشد الفتحات يُحجز عبر الفتحات فقط (لا حجز مزدوج).

**فُحص ووُجد محصّناً:** حقن PostgREST/SQL (supabase-js مُعامَل)، RBAC الإداري، ملكيّة
الموارد (IDOR)، القوائم البيضاء، قصّ العمولة/VAT، الجلسة/الكوكيز/HSTS، حدود المعدّل،
التقييمات (حجز مكتمل + واحد)، رفع الملفّات (WebP + قائمة بيضاء + SVG→octet-stream)،
تكلفة الذكاء (حدّ IP)، ولا أسرار في المستودع. 62 اختباراً ناجحاً.

---

## 🖥️ تدقيق الخادم والعمليّات — 2026-06-22

**أُصلح ونُشر على الخادم:**
- **الجدار الناري:** منفذا Postgres (5432/6543) كانا منشورَين على `0.0.0.0` بلا حظر
  (Docker يتجاوز UFW). أُضيفا إلى قاعدة DROP في DOCKER-USER مع
  `guideon-firewall.service` — الآن 22/80/443 فقط مكشوفة خارجيّاً (لا IPv6 عام).
- **النسخ الاحتياطي:** كان يحفظ `public` فقط؛ صار يشمل مخطّط `storage` **وصور
  التخزين الفعليّة** (٣٢٩ ميغا) يوميّاً ٠٢:٣٠، احتفاظ ١٤ يوماً.
- **سجلّات Docker:** كانت غير محدودة؛ ضُبط التدوير (10m×3) عبر daemon.json.
- نسخ مرجعيّة للسكربتات في `deploy/ops/`.

**سليم:** إعادة التشغيل التلقائي (`unless-stopped`)، Swap، تجديد TLS اليومي.

**الفجوة المتبقّية (تحتاج قرارك):** النسخ الاحتياطي **محليّ على نفس الخادم** — لا
نسخة خارجيّة (offsite). يُوصى بـrclone إلى S3/Backblaze/Drive أو scp لخادم آخر.

---

## 🔬 تدقيق المناطق المتبقّية — 2026-06-22

- **المصادقة:** رموز ٢٥٦-بت، bcrypt، تجديد جلسة، رسالة موحّدة، إعادة تعيين بلا
  تسريب، Google يُتحقّق خادميّاً، 2FA (TOTP window 1 + رموز نسخ تُستهلك + حدّ
  محاولات). **أُصلح:** تسريب توقيت عند الدخول لبريد غير موجود (bcrypt وهمي).
- **الحسابات المالية:** الخزانة متّسقة (عمولة المدفوع فقط + توازن صحيح + تقريب
  ٣ خانات)، المصروفات بموجب فقط، تدقيق مُسجَّل. سليمة.
- **SSE:** البثّ خلف `requireLogin`. **XSS الواجهة:** كل المحتوى عبر
  `escHtml`/`gdSafeUrl`. **البريد:** Resend API (لا حقن ترويسات). **SEO:** robots +
  sitemap سليمة. **المكتبات:** ١٩ ثغرة متوسّطة في أدوات الرصد فقط (خطر منخفض).

---

## 🧪 اختبار حيّ للمميزات — 2026-06-22

اختبار شامل على الإنتاج:
- **٤٣ صفحة** كلّها 200 · المسارات العامّة (مرشدون/شركات/فرق/باقات/إحصاءات) تعمل.
- **الحماية:** كل المسارات المحميّة 401، كل مسارات الإدارة 401.
- **تدفّق كامل:** تسجيل → دخول → جلسة → منطق الحجز (تاريخ ماضٍ→400، حقول ناقصة→400)
  → حجوزاتي/إشعارات/محادثات/2FA = 200 (بحساب تجريبي حُذف بعدها).
- **الذكاء:** الشات-بوت يردّ فعليّاً، مخطّط الرحلات 200. البحث بالفلاتر 200.
  sitemap (٩٣ رابط) + robots + صفحات SEO + PWA = 200.
- **أُصلح (اكتُشف بالاختبار):** التسجيل/الدخول/me/جوجل/الملف/2FA كانت **تُسرّب
  رموزاً حسّاسة** (emailVerifyToken/resetPasswordToken/fcm_token) → دالّة
  `publicUser` موحّدة في `src/utils/sanitizeUser.js`. مؤكَّد مُصلَح حيّاً.

---

## 🎬 رفع الفيديو + 🔒 إصلاح أمني حرِج — 2026-06-22

**ميزة الفيديو:** يستطيع المرشدون والشركات والباقات رفع **فيديو من الجهاز** (لا
يوتيوب فقط) عبر مكوّن `gd-video-upload.js` الذي يعرض **الشروط والحجم** (MP4/WebM/MOV،
حتى ٥٠ ميغا) مع شريط تقدّم. يُحفظ في عمود `videoFileUrl` (migration 044) ويُعرض
بمشغّل HTML5 في الملفّات العامّة وصفحة الرحلة.

**🔴 ثغرة حرِجة اكتُشفت وأُصلحت:** واجهات المرشدين/الشركات/الفرق العامّة كانت تكشف
`resetPasswordToken` و`twoFactorSecret` و`emailVerifyToken` و`fcm_token` لكل
مزوّد لأي زائر (خطر استيلاء على الحساب). أُصلح جذريّاً في `sanitizeContact`
(تجريد الرموز دائماً) + `publicUser` في باقي المواضع. مؤكَّد: صفر تسريب حيّاً.
احترازيّاً أُبطلت رمزَا إعادة تعيين نشطان. سرّ 2FA الإداري لم يُكشف (الإدارة خارج
الواجهات العامّة).

---

## 🛡️ مراقبة + تدوير مفاتيح Supabase — 2026-06-22

- **المراقبة (#3):** سكربت يفحص كل ٥ دقائق (الموقع، الحاويات، القرص، شهادة TLS)
  ويُرسل تنبيهاً فوريّاً إلى admin@guideon.om عبر Resend عند أي خلل. يُنصح بإضافة
  مراقب خارجي (UptimeRobot) لرصد سقوط الخادم كاملاً.
- **تدوير المفاتيح (#2):** مفاتيح Supabase الذاتيّة كانت لا تزال الافتراضيّة
  الشهيرة. دُوّرت إلى قيم عشوائيّة فريدة (JWT_SECRET + anon + service) مع نسخ
  احتياطيّة وتحقّق حيّ ناجح. **ملاحظة:** كلمة مرور Postgres لا تزال الافتراضيّة
  (محميّة بالجدار الناري) — تدويرها أعلى خطراً ويُترك كخطوة لاحقة عند الرغبة.

---

## 💳 تفعيل المدفوعات — Paymob (Oman) — 2026-06-23

بوّابة الدفع الفعّالة الآن **Paymob Accept (عُمان)** عبر **Unified Checkout
(Intention API)** — لأنّ حساب Paymob يستخدم Secret/Public keys (وزر API Key
الكلاسيكي معطوب في بوّابتهم). المتغيّرات على الخادم: `PAYMENT_PROVIDER=paymob`،
`PAYMENTS_ENABLED=true`، `PAYMOB_SECRET_KEY`، `PAYMOB_PUBLIC_KEY`،
`PAYMOB_INTEGRATION_ID=69823` (card)، `PAYMOB_HMAC_SECRET`.
- Callback (خادم): `https://guideon.om/api/payments/paymob/callback` (تحقّق HMAC SHA512).
- إرجاع المستخدم: `https://guideon.om/checkout-success.html`.
- التدفّق: intention (Secret Key) → unifiedcheckout (Public Key) → webhook يربط
  الحجز عبر special_reference ويطابق المبلغ ثمّ يُعلّم مدفوعاً.
- **مُختبَر حيّاً** طرفاً لطرف (إنشاء جلسة دفع حقيقيّة نجح). المدفوعات **حيّة**.
- Thawani يبقى كبديل (PAYMENT_PROVIDER=thawani). سكربت التفعيل: `/opt/paymob-setup.sh`.

---

## 📱 تطبيق الجوّال (v126) + إصلاحات ويب حرِجة — 2026-07-08

**تطبيق الجوّال — أُرسل الإصدار 126 لمراجعة Google Play (قيد المراجعة):**
- **الموقع (geolocator):** زر "موقعي" على الخريطة الأصليّة يوسّطها على المستخدم
  ويضع علامة "أنت هنا" ويعرض المسافة بالكيلومتر لكل مرشد/شركة. الاستخدام **محلّي
  على الجهاز فقط** (لا يُرسَل للخادم) — لذا لا يتطلّب إعلاناً في نموذج Data safety.
- **الكاميرا:** رفع الصور مباشرة من الكاميرا داخل WebView (ملف/رحلات/تقييمات/
  مرفقات). اختيار المعرض عبر Android Photo Picker بلا إذن.
- **⚠️ درس سياسة Play:** حُذفت `READ_MEDIA_IMAGES`/`READ_EXTERNAL_STORAGE` لأن
  فحص Play رفضها (الرفع غير المتكرّر لا يبرّر إذن الوصول الدائم للوسائط). أُبقي
  `CAMERA` + الموقع فقط.
- **ترقية Firebase:** `firebase_core ^2.27→^3.8`, `firebase_messaging ^14.7→
  ^15.1.6` لحلّ تعارض تبعيّات كسر بناء Codemagic (تعارض `platform_interface`).
- إصلاحات سابقة في نفس الجولة: قبول/رفض الحجوزات (PUT→PATCH في bookings)،
  تسريب الجلسة عبر المتصفّح الخارجي (WebSheet مشترك)، ظهور الشركات في البحث/
  الخريطة، عرض مدّة الرحلة (0 أيام).

**إصلاحات الويب الحرِجة (منشورة حيّاً):**
- **الإشعارات كانت تطرد المستخدم من المنصّة:** رابط إشعار الرسالة كان يُبنى من
  نوع **المُرسِل** (`req.session.userType`) لا المُستقبِل، فيصل المرشد لرابط لوحة
  السائح. أُصلح إلى `toUser.userType` في `messagesController`. **والأهمّ:** حرّاس
  اللوحات كانوا يوجّهون أي مستخدم على لوحة خاطئة إلى `/login.html` (يبدو تسجيل
  خروج). أُصلح في tourist/guide/company-dashboard: يُطرد لتسجيل الدخول فقط إن كان
  غير مسجّل فعلاً، وإلا يُعاد توجيهه للوحته الصحيحة مع الحفاظ على `#messages`.

**⚠️ درس نشر مهمّ:** اكتُشف أن `docker compose up -d --build` فشل مرّة في `npm ci`
(رمز 146 = انقطاع شبكة عابر مع سجل npm)، فبقيت الحاوية بكود قديم رغم نجاح
`git pull` — أي أن webhook النشر **فشل صامتاً**. الحلّ الفوري كان `docker cp`
للملفّات ثمّ إعادة بناء ناجحة وإعادة إنشاء الحاوية. **القاعدة: بعد أي دفع، تحقّق
أن التغيير ظهر فعليّاً على guideon.om (لا تفترض نجاح النشر التلقائي).**

---

## 🚀 نشر محصّن + تنبيهات بريد موثوقة — 2026-07-09

**المشكلة المكتشفة:** النشر التلقائي (webhook) كان **يفشل صامتاً** — `deploy.sh`
القديم سطرٌ واحد `docker compose up -d --build`، فإذا تعثّر `npm ci` بانقطاع شبكة
عابر (رمز 146) توقّف البناء وبقيت الحاوية القديمة تخدم كوداً قديماً، دون تنبيه.
لهذا "نشرنا" إصلاحات ولم تظهر فعليّاً.

**`deploy.sh` الجديد** (على `/opt/deploy.sh` + نسخة متتبَّعة في `deploy/deploy.sh`):
1. **إعادة محاولة البناء 3×** — يتجاوز انقطاعات npm العابرة.
2. **تحقّق صحّة** بعد النشر (يتأكّد أن guideon.om يردّ HTTP 200).
3. **تنبيه بريد عند أي فشل** (git pull / بناء / تشغيل / صحّة) — لا فشل صامت.

**تفاصيل تنبيه البريد (Resend) — دروس مهمّة:**
- **لا تُحمّل `.env` بـ `source`**: قيمة `EMAIL_FROM=Guideon <noreply@guideon.om>`
  (مسافات و`<>` غير مقتبسة) تُفشِل `source` وتترك `RESEND_API_KEY` فارغاً. الحلّ:
  استخراج المفاتيح المطلوبة بـ `grep+cut` فقط.
- **`admin@guideon.om` ليس صندوقاً حقيقيّاً** — كان يرتدّ فحظره Resend
  (`suppressed`)، ولا توجد واجهة API لرفع الحظر (يدويّاً من لوحة Resend فقط).
- **Resend يحظر الرسالة كاملةً إن كان أي مستلِم محظوراً** — فبريد جماعي لعنوانين
  (محظور + سليم) يسقط للاثنين. الحلّ: `alert()` يرسل **رسالة منفصلة لكل عنوان**.
- **الوجهة النهائيّة (مؤكَّدة `delivered`):** `ADMIN_ALERT_EMAIL=hh92hh@guideon.om,
  Hh95255450hh@hotmail.com`، و`ADMIN_EMAIL=hh92hh@guideon.om`. (التطبيق يقرأ
  `ADMIN_ALERT_EMAIL` مفصولاً بفواصل في `emailService.js` لتنبيهاته الإداريّة أيضاً.)

**قاعدة ذهبيّة:** بعد أي `git push`، تحقّق أن التغيير ظهر فعليّاً على guideon.om —
لا تفترض نجاح النشر التلقائي.

---

## 🔧 إصلاح جذري لتعطّل البناء التلقائي — 2026-08-22

**العَرَض:** كل نشر تلقائي (webhook) كان **يفشل** — رسائل «Guideon deploy FAILED»
بريديّة، وأي تعديل في كود الخادم لا ينزل (اضطررنا لالتفاف يدوي عبر `docker cp`).

**السبب الجذري (سببان مجتمعان):**
1. **تضخّم ذاكرة Docker المؤقتة:** `Build Cache` بلغ **57GB** (2528 طبقة) +
   صور **49GB** = ~106GB مخلّفات لم تُنظَّف قط. أرهقت الخادم: **load = 45.8**
   على 8 أنوية، و**170MB ذاكرة حرة فقط** → كل بناء يتبادل مع القرص فيتجاوز
   مهلة الـ webhook (chromium وحده استغرق 30 دقيقة).
2. **`deploy/nginx.conf` معدّل محليًا على الخادم بشكل دائم** (إعداد خاص لم
   يُثبَّت) → `git pull` يرفض الدمج حتى لا يطمسه، فيفشل النشر عند خطوة السحب.

**الإصلاح:**
- تنظيف `docker builder prune -af` + `image prune -af` → حُرِّر **~99GB**
  (الذاكرة المؤقتة 57GB→0، القرص 38G→24G، الحمل 45.8→**0.45**، الذاكرة الحرة→**5.5GB**).
- تثبيت `nginx.conf` على الخادم (commit) لتنظيف شجرة git → السحب لم يعد يتعارض.
- **وقاية دائمة:** أُضيف إلى `deploy.sh` تنظيف تلقائي أسبوعي
  (`docker builder/image prune -af --filter until=168h`) يعمل **بعد** فحص
  الصحة الناجح، **في الخلفية**، ولا يُفشل النشر أبدًا.

**النتيجة:** النشر التلقائي عاد يكتمل في **~30 ثانية** (كان يفشل بعد 30–40 دقيقة).
أي `git push` يُنشر ذاتيًا الآن دون تدخّل، والذاكرة المؤقتة تُنظَّف نفسها أسبوعيًا.

**درس:** راقب `docker system df` دوريًا؛ تضخّم الـ Build Cache قاتل صامت على
خادم بذاكرة محدودة. ولا تعدّل ملفات متتبَّعة (nginx.conf) مباشرة على الخادم —
عدّلها في المستودع وادفعها، وإلا تعارض السحب التالي وفشل النشر.

---

## ✅ الإصدار 126 منشور وحيّ على Google Play — 2026-07-09

**تطبيق الجوّال — الإصدار 126 (1.0.0) الآن "متوفّر على Google Play"** (نُشر
9 يوليو 2026، 12:23 ص). الطرح **الكامل 100%** في **176 دولة**، 19,009 جهاز متوافق.
الميزات الحيّة للمستخدمين: الموقع + أقرب المرشدين، رفع الصور من الكاميرا،
إصلاح قبول/رفض الحجوزات، إصلاح طرد الإشعارات (الويب).

**⚠️ ملاحظة تشغيليّة مهمّة — النشر المُدار (Managed Publishing) مُفعّل:** بعد
موافقة Google على أي إصدار، يبقى **معلّقاً** في صفحة "نظرة عامة على النشر" حتى
تُضغط "نشر تغيير واحد" **يدويّاً**. لا يصل المستخدمين قبل ذلك. تذكّر هذه الخطوة
لكل تحديث قادم حتى لا يبقى إصدار مُوافَق عليه دون نشر. (يمكن إيقاف هذا الخيار من
الإعدادات إن رُغب بالنشر التلقائي فور الموافقة.)

---

## 🚀 الإصدار 1.1.0 (build 17) — تحوّل "مرآة المنصة" — 2026-07-16

**أكبر تحديث للتطبيق:** التطبيق الآن يعرض منصة guideon.om نفسها داخل تبويبات
WebView متزامنة الجلسة + شريط تنقّل سفلي أصلي — نفس الأزرار والمحتوى تمامًا،
فلا تكرار شاشات أصلية يجب مزامنتها مع الويب.

**ما يصلحه/يضيفه للمستخدم:**
- إصلاح جذري لعرض الرحلات والصور وملفات الشركات (كانت لا تظهر في الشاشات الأصلية)
- رفع الصور يعمل من داخل التطبيق (file_picker + setOnShowFileSelector)
- تسجيل توكن الإشعارات تلقائيًا بعد تسجيل الدخول (تحقّقنا بإشعارين وصلا فعلًا)
- شاشة "لا يوجد اتصال" أنيقة مع زر إعادة المحاولة
- تحميل كسول للتبويبات (سرعة فتح أفضل بكثير)
- أيقونة تطبيق جديدة بخلفية بيضاء (الشعار الأصلي)
- خيار "متاحة كل يوم" للرحلات (منصة + تطبيق)

**درس بناء مهم (Gradle):** بناء AAB فشل 3 مرات — `file_picker` يثبّت
compileSdk 34 بينما `flutter_plugin_android_lifecycle` يتطلب 36. الحل النهائي
في `mobile/android/build.gradle.kts`: كتلة `subprojects` تفرض 36 عبر
`afterEvaluate` محمية بـ`state.executed` (لأن `evaluationDependsOn(":app")`
يجعل `afterEvaluate` العادي يرمي "already evaluated"، و`plugins.withId` يعمل
مبكرًا فتطغى قيمة المكتبة).

**تذكير:** بعد موافقة Google — اضغط "نشر" يدويًا (Managed Publishing).

---

## 💰 تسعير مرن بالشرائح والأعمار للرحلات — 2026-08-23

**الطلب:** صاحب شركة احتاج تسعيرًا مثل TripAdvisor (١-٢ = ٦ ر.ع · ٣-٤ = ٨ · ٨+ = ١.٥/شخص
+ سعر طفل + مجاني تحت ٦) — والنموذج القديم (سعر واحد / أساس لشخصين) لم يكفِ. كما كان
حقل المدة في لوحة الشركة نصًّا يخزّن «٣٠» دقيقة كـ٣٠ **يومًا**.

**الحل (للمنصّة كلها — مرشدون وشركات):**
- **محرّك تسعير جديد** `src/domain/bookingRules.js`: `pricing_mode:'tiered'` مع
  `pricing_tiers` (شرائح حسب عدد البالغين، flat للمجموعة أو per_person) + `child_price`
  + `free_under_age`. النموذج القديم `'simple'` (أساس لشخصين + إضافي) يبقى الافتراضي بلا كسر.
- **١٣ اختبار وحدة** على أمثلة الشركة الحقيقية (كلها تمر؛ الإجمالي ٨٣).
- **الخادم مصدر الحساب النهائي** (لا يثق بسعر العميل)؛ تنقية صارمة للجداول القادمة.
- لوحتا **الشركة والمرشد**: محرّر شرائح + أعمار + مدة (أيام/ساعات/دقائق).
- **صفحة الرحلة**: خانتا بالغين/أطفال + جدول أسعار + حساب فوري (تُرسِل adult/childCount
  وتحذف variantPrice ليستخدم الخادم محرّك الشرائح).

**قاعدة البيانات على ODP فقط** (Supabase ذاتية الاستضافة، حاوية `supabase-db`). الجدول
اسمه **`tour_packages`** (لا `packages`). الترحيل 055 طُبّق مباشرةً عبر
`docker exec supabase-db psql -U postgres -d postgres` — أضاف ٦ أعمدة
(pricing_mode/pricing_tiers/child_price/child_age_min/child_age_max/free_under_age).
تحقّقنا حيًّا: حفظ + حساب صحيح (٢=٦، ٨=١٢، ٢+٣أطفال=١٢) ثم أُعيدت رحلة الاختبار لـsimple.

**درس:** لأي ترحيل مستقبلي — القاعدة على ODP فقط، شغّله عبر
`docker exec supabase-db psql -U postgres -d postgres -c "…"`، وأسماء الجداول الفعلية
قد تختلف عن أسماء النماذج في الكود (packages ← `tour_packages`).
